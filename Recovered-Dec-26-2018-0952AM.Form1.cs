    ...
    public partial class Form1 : Form
    {
        ...
        class priorityqueue
        {
            ...
            public static void shortestpath(double xstart, double ystart, double xend, double yend, double r, int filenum)
            {
                ...
                FileStream finalfile = new FileStream(@"C:\Users\Lenovo\Desktop\sample\output" + filenum + ".txt", FileMode.Append, FileAccess.Write);
                ...
                while (stack_nodes.Count != 0)
                {
                    ...
                    Point p = new Point();
                    p.X = (int)nodeparent.x;
                    p.Y = (int)nodeparent.y;
                    listpoint.Add(p);
                    //Console.Write(stack_nodes.Pop() + " ");
        ...
        private void show_Click(object sender, EventArgs e)
        {
            ...
            pp.X = 4500;
            pp.Y = 1347;
            this.CreateGraphics().DrawLine(new Pen(Brushes.Blue, 4), pp, ppp);
            for (int i = 0; i < listpoint.Count; i++)
            {
                ...
                if (i != 0)
                {
                    ...
                    this.CreateGraphics().DrawLine(new Pen(Brushes.Blue, 4), listpoint[i - 1], listpoint[i]);
                }
                else
                {

                    this.CreateGraphics().DrawLine(new Pen(Brushes.Black, 2), listpoint[i], listpoint[i]);
                }
